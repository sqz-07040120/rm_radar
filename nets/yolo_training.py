...
class YOLOLoss(nn.Module):    
    ...
    def get_in_boxes_info(self, gt_bboxes_per_image, expanded_strides, x_shifts, y_shifts, total_num_anchors, num_gt, center_radius = 2.5):
        ...
        #(shifts[0] + 0.5) * expanded_strides_per_image:得到单元格中心点坐标
        #-------------------------------------------------------#
        ...
        #(gt_bboxes_per_image[:, 0] - 0.5 * gt_bboxes_per_image[:, 2])：得到左上，右下坐标
        #-------------------------------------------------------#
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        topk_ious, _ = torch.topk(pair_wise_ious, n_candidate_k, dim=1)#torch.topk():求tensor中某个dim的前k大或者前k小的值以及对应的index。
